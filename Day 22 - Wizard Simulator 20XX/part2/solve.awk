...
END {
	...
	do {
		...
		if (min_mana_spent && state["mana_spent"] >= min_mana_spent)
			continue;
		if (survive_through_hard_mode(state) && apply_effects(state)) {
...
# Apply the hard difficulty mode rule. Returns 1 if we're still alive, 0
# otherwise (the game is over).
function survive_through_hard_mode(state) {
	state["my_hp"] -= 1;
